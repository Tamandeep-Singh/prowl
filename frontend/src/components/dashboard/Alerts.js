            ...
            const fetchAlerts = async () => {
              ...
              response.result.forEach(alert => {
                alerts.push({
                    ...
                    score: alert.score,
                    date_added: new Date(alert.createdAt)
                });
              });
              setRows(alerts);
            };
  ...
  <Dialog open={showReportPopup} onClose={onCloseReportPopup} fullWidth maxWidth="sm">
    ...
    <Dialog open={showInformationPopup} onClose={onCloseShowInformationPopup} fullWidth maxWidth="sm">
      ...
      <DialogContent sx={{ whiteSpace: "pre-wrap", wordBreak: "break-word", overflowX: "hidden", overflowY: "auto", maxHeight: "70vh"}}>
         <p style={{ marginTop: -3, marginBottom: 0 }}>This page lists all alerts generated by the API after processing endpoint telemetry from the agent scripts. Alerts can range from Low, Medium and High for their <strong>Severity</strong> and have a varying <strong>Score.</strong></p>
         <p><strong>Automated alerts</strong> with a severity tagged as "High" will be sent via Email and Discord (through Webhooks).</p>
         <p>For any alert, you are able to generate an <strong>AI Report</strong> through the <strong>"Generate Report"</strong> button under "Actions", which can provide potential remediation strategies or further insight into the alert.</p>
      </DialogContent>
      <DialogActions>
