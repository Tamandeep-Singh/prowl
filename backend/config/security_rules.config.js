const rules = {
    processes: [
       ...
       { detection: "Netcat", filter: "/^(?:[A-Za-z]:\\|\/)?(?:[\w.-]+[\/\\])*?(nc|netcat|ncat)(?:\.exe)?(?:\s+.*)?$/", score: 4, message:"Netcat was used, potentially for a bind / reverse shell and network mapping.", field: "command"},
       ...
       { detection: "Cron", filter: "/^(?:[A-Za-z]:\\|\/)?(?:[\w.-]+[\/\\])*?crontab(?:\.exe)?(?:\s+.*)?$/", score: 5, message:"Cron was used, potentially to establish persistence via ", field: "command"}
    ],
    files: [
    ...
    network_connections: [
        { detection: "Command Network Filter", filter: "^(nc|netcat|ncat|bash|python|wget|curl|ssh|telnet|powershell|ftp)(?:\.exe)?$", score: 3, message:"Unusual command detected to spawn a network process.", field: "command"}
    ],
    whitelist: {
        processes: [],
        files: [],
    ...
    blacklist: {
        ...
        processes: [{ detection: "Netcat (Blacklist)", filter: "/^(?:[A-Za-z]:\\|\/)?(?:[\w.-]+[\/\\])*?(nc|netcat|ncat)(?:\.exe)?(?:\s+.*)?$/", message:"Blacklisted Process (netcat) detected!", field: "command" }],
